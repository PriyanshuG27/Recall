...
Create backend/modal_apps/modal_llama.py — Llama 3 8B summarisation:
...
- GPU type: T4 (cheapest Modal GPU; sufficient for Whisper large-v3 and Llama 3 8B).
...
[ ] modal_llama.py returns valid summary for 200-word input text
```
...
```
Create backend/services/ai_cascade.py — the 5-tier fallback orchestrator.
...
Tier 1: Groq API (timeout=20 s)
Tier 2: Gemini 3.1 Flash-Lite (timeout=20 s)
Tier 3: Ollama (timeout=60 s, skip if OLLAMA_HOST not set)
Tier 4: Return CascadeResult(success=False, tier_reached=4)
...
- Tier 4 exhaustion must write to dead_letter_queue BEFORE sending user notification.
...
[ ] Mock Tiers 0-3 failure → dead_letter_queue entry created
...
8. INSERT into quizzes (if quiz_data returned from Llama).
9. Bot reply:
   "🎙 Transcribed:\n{first 200 chars}...\n\n📝 Summary:\n{summary}\n\nSaved ✓"
10. Delete /tmp file.
...
Map-Reduce flow:
  ...
  Reduce: pass all 5 summaries to Llama 3:
...
Job 2 — louvain_clustering (daily 02:00 UTC):
  For each user with >= 10 new items since last run:
    ...
    For each community: compute centroid, generate label via Llama3
...
[ ] modal app list → 3 apps deployed (whisper, llama, embed)
...
3. Modal
   ...
   → modal deploy backend/modal_apps/modal_llama.py
...
| 012 | Modal Whisper/Llama/Embed | P2 | ☐ |
