    ...
    async with worker_semaphore:
        ...
        try:
            ...
            if task.get("is_batch"):
                await process_batch_task(task, user_id, chat_id)
                return

            # 4. Standard single-item processing routing
            await process_single_item(task, user_id, chat_id, is_batch_item=False)
...
async def process_batch_task(task: Dict[str, Any], user_id: int, chat_id: str) -> None:
    ...
    for item in batch_items:
        ...
        try:
            item_id = await process_single_item(sub_task, user_id, chat_id, is_batch_item=True)
            if item_id:
    ...
    for save in final_saves:
        ...
        await send_telegram_message(chat_id, bot_reply, reply_markup=build_recall_keyboard(save["id"]))

        # Dynamic Context Note Why Question
        try:
            async with _pool.connection() as conn:
                async with conn.cursor() as cur:
                    await cur.execute("SELECT context_prompt FROM items WHERE id = %s AND user_id = %s;", (save["id"], user_id))
                    row = await cur.fetchone()
            ...
            await send_context_prompt_with_checks(chat_id, user_id, save["id"], context_prompt)
...
async def process_single_item(task: Dict[str, Any], user_id: int, chat_id: str, is_batch_item: bool = False) -> Optional[int]:
    ...
    if content_type == "text":
        ...
        cascade = AICascade()
        ai_res = await cascade.summarise(text_content, chat_id, mood_category=mood_category)
        ...
        try:
            ...
            try:
                ...
                if not is_batch_item:
                    ...
                    await send_telegram_message(chat_id, bot_reply)
                return getattr(dup_exc, "item_id", None)
